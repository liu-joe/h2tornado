...
BODY_STR = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in xrange(1000))
...
if __name__ == '__main__':
    for num_requests in [1, 10, 100, 1000, 5000]:
        ...
        for poolsize in [1, 10, 25, 50, 100]:
            hyper_bench_time = run_requests_hyper_benchmark(num_requests, poolsize, 0)
            print "requests/hyper %d threads: Did %s requests in %s seconds, %ss/op" % (poolsize, num_requests, hyper_bench_time,float(hyper_bench_time) / num_requests)
            best_hyper_bench_time = min(best_hyper_bench_time, hyper_bench_time)
